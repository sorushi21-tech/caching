...
public class TransactionService {
  ...
  /**
   * Creates a new transaction and evicts the transactions cache, because the cached list is now
   * stale.
   */
  ...
  public Transaction createTransaction(TransactionRequest request) {
    ...
    log.info(
        "Saved transaction id={} category={} currency={} amount={} — transactions cache evicted",
        saved.id(),
        saved.category(),
        saved.currency(),
        saved.amount());
  ...
  /**
   * Returns all transactions, cached via Caffeine. The cache automatically expires entries after
   * the configured TTL (60s). Also evicted explicitly when a new transaction is created.
   */
