...
public class FrankfurterClient {
    ...
    public FrankfurterClient(
            ...
            @Value("${frankfurter.api.base-url}") String frankfurterBaseUrl
    ) {
    ...
    public Optional<BigDecimal> getRate(String fromCurrency, String toCurrency) {
        ...
        String uri = UriComponentsBuilder
                .fromUriString(frankfurterBaseUrl)
                .path("/latest")
        ...
        try {
            ...
            FrankfurterLatestResponse response = restTemplate.getForObject(uri, FrankfurterLatestResponse.class);

            if (response == null || response.rates() == null) {
                log.warn("Frankfurter rate response was empty for from={} to={}", normalizedFrom, normalizedTo);
            ...
            if (rate.isEmpty()) {
                log.warn("Frankfurter response did not include requested symbol for from={} to={}", normalizedFrom, normalizedTo);
            } else {
                log.debug("Frankfurter returned rate {} for from={} to={}", rate.get(), normalizedFrom, normalizedTo);
            }

            return rate;

        } catch (RestClientException ex) {
            log.warn("Failed to fetch rate for from={} to={} due to client error", normalizedFrom, normalizedTo, ex);
            return Optional.empty();
        }
    }
